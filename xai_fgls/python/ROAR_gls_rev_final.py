        ...
        for percent in tqdm(percentiles):
            ...
            if attribution == 'normal':
                if percent == 0.0:
                    ...
                    test_image = render.save_image([raw_image, prdigit],'../{}_ROAR_check_point_{}.png'.format(attribution, percent))
...
#     ratio = 0.1
    ...
    for _ in range(3):
        ...
        for v in attribution_methods:
            logger.info("=============================================")
            logger.info("[{} ROAR start] : {}".format(v, str(datetime.datetime.now())))
            st = time.time()
            res = []

            for p in percentiles:
                logger.info("=============================================")
                logger.info("[{} ROAR percentile] : {}".format(v, p))
                start = time.time()
...
#                 ypred = DNN.forward(Xtest)
    ...
    return res_mean
logger.info("=======================================================")
logger.info("[ROAR start] : {}".format(str(datetime.datetime.now())))
...
logger.info("================final check ROAR is above================")
logger.info(results)